...
class TwitterFriends{
	...
	function __construct($id, $cursor = null, $last_update = null){
		...
		$this->id = $id;
		$this->ratelimit = twitter_get_ratelimit('friends', '/friends/ids');
	...
	private function get_followers(){
		...
		$response = twitter()->get('friends/ids', $params);
		if(!is_object($response) || !isset($response->ids)){
			return $this->ids;
		}
		$this->ratelimit['remaining'] = twitter_ratelimit()->used('friends', '/friends/ids');
